...
def evaluate(findings, policy: dict) -> dict:
    ...
    if counts.get("critical", 0) >= critical_limit:
        reasons.append(
            f"Critical finding threshold reached: {counts.get('critical', 0)} >= {critical_limit}"
        )

    if counts.get("high", 0) >= high_limit:
        reasons.append(
            f"High finding threshold reached: {counts.get('high', 0)} >= {high_limit}"
        )

    scored = [f for f in findings if f.risk_score >= min_score]
    if scored:
        reasons.append(
            f"{len(scored)} finding(s) have risk score >= {min_score}"
        )
